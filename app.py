...
st.write("To balance interpretability and completeness, a smaller baseline model is used alongside a more extended version. While the baseline model focuses on a few key features, the extended model includes a wider set of variables as a robustness check. In both cases, the overall conclusion remains the same: the relationships are weak. Extending the model to include a wider set of features does not meaningfully improve its explanatory power, with results remaining similarly weak.")
st.write("It’s also important to be clear about the limitations of this approach. With only around 220 tracks, the dataset is relatively small, and there is a risk of omitted variable bias. Factors such as marketing, playlist placement, and broader cultural context are not included in the model, but are likely to influence success. If these omitted factors are correlated with the included audio features, for example, more commercially promoted tracks may also be more polished or more “pop-oriented”, the estimated relationships may partly reflect these underlying influences instead of purely musical characteristics.")
st.image("output/figures/regressions/scaled_track_coefficients.png") # show an image showing the regression coeficients
