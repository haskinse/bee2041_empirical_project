...
st.write("Taylor Swift is one of the most successful music artists of all time. She’s sold over 100 million album units, multiple of her songs have surpassed one billion streams, and she’s the highest-grossing live music artist. But what actually drives that success? Is it something in the music itself, or something else entirely?")
st.write("This blog looks at the musical features of her songs to see whether certain characteristics are linked to success. It asks whether patterns in the data can help explain why some tracks perform better than others.")
...
st.write("The dataset used in this project combines several sources. Album-level measures of commercial success were scraped directly from Wikipedia using Python. A request was sent to the page with a browser-style header to avoid access issues, before using BeautifulSoup to parse the HTML and extract the relevant tables. These tables were then converted into pandas DataFrames and cleaned to ensure consistent formatting across albums.")
st.write("Track-level features were taken from a Kaggle dataset, which provided information on individual songs. However, this data was not perfectly aligned with other sources, so track names were standardised using string cleaning techniques, such as removing brackets and normalising formatting, to improve consistency.")
st.write("The cleaned track data was then matched with Spotify information to ensure accurate identification of songs, before play count and listener statistics were added from Last.fm. This created a combined dataset linking musical features to measures of commercial success at both the track and album level.")
st.write("Some challenges arose during this process, particularly due to inconsistencies in naming conventions and formatting across sources, as well as the presence of footnotes and missing values in scraped tables. These issues were addressed through a combination of automated cleaning and manual checks where necessary.")
...
st.write("In this project, success is measured in a few different ways. At the album level, it’s based on certified units and total sales. As a rough proxy for cultural impact, Wikipedia page length is also used, although this is only an approximation, so is not heavily used in this project. At the track level, success is measured using listener and play counts from Last.fm.")
st.write("Using a combination of these measures makes success easier to quantify and gives a more balanced view. That said, there are still limitations, for example other potential success factors like critical reception are left out here.")

st.divider() # add a divider between sections

st.header("Which of Taylor Swift’s albums have been the most successful?") # add heading and text
st.write("Taylor Swift’s most commercially successful album, by both sales and certified units, is her fifth album, *1989*, followed by her second album, *Fearless*. When combined with the re-released Taylor’s Version, *1989* has over 20 million certified units and around $10 million in certified sales.")
st.write("Looking at Wikipedia page length instead, *Folklore* comes out on top, although it’s closely followed by *1989*. This suggests that despite slightly weaker commercial performance, *Folklore* may have had a stronger cultural impact, or at least generated more discussion. However, this also highlights a potential limitation of using Wikipedia page length as a proxy for success. As Taylor Swift became more famous over time, more people write about her albums regardless of their commercial performance. In that sense, the relationship becomes slightly circular, greater fame leads to more coverage, which then makes later albums appear more culturally significant simply because there is more information available about them.")
st.write("Overall, the most striking feature of the data is how consistent her success has been across her career. Even her lowest-performing album, *Evermore*, still has over four million certified units, going 4× platinum in the United States. Unlike many other artists, Swift has largely avoided a true flop era.")
